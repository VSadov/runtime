...
//
// Linux implementations of POSIX semaphore work directly on top of futex and
// can be considerably faster than a custom implementation on top of {mutex, condvar}.
// Otherwise we fall back to a custom implementation on top of {mutex, condvar}, which is more portable.
//
#ifdef TARGET_LINUX
...
void SystemNative_NativeSemaphore_Wait(void* semaphore)
{
    assert(semaphore != NULL);
...
int32_t SystemNative_NativeSemaphore_Release(void* semaphore)
{
    assert(semaphore != NULL);

    int error = sem_post((sem_t*)semaphore);
    assert(error == 0 || error == EOVERFLOW);
...
int32_t SystemNative_NativeSemaphore_Release(void* semaphore)
{
    ...
    assert(pSemaphore->Count <= pSemaphore->MaxCount);
