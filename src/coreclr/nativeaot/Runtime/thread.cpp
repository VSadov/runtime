...
UInt32_BOOL Thread::HijackCallback(HANDLE /*hThread*/, PAL_LIMITED_CONTEXT* pThreadContext, void* pCallbackContext)
{
    ...
    return false; //pThread->InternalHijack(pThreadContext, NormalHijackTargets);
}
...
bool Thread::InternalRedirect(PAL_LIMITED_CONTEXT * pSuspendCtx)
{
    ...
    // void* pvRetAddr = (void*)pSuspendCtx->GetIp();
    // m_pvHijackedReturnAddress = pvRetAddr;
    ...
    // we have redirected this thread, it no longer needs return hijacking.
    CrossThreadUnhijack();
