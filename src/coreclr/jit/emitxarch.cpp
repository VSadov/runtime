    ...
    switch (insFmt)
    {
       ...
       // the addr can be encoded as pc-relative address.
        ...
        DONE_CALL:
            ...
            // TODO: VS do we care about return regs if tailcalling?
            //      or anything that follows from here till break?
            ...
            if (ins == INS_call)
            {
                ...
                // TODO: VS do we need to record a tailcall at all?
                //       the next inst is not reachable from here, so why bother.

                if (recCall || args)
                {
                    // For caller-pop, all arguments will be popped  after the call.
                    // For callee-pop, any GC arguments will go dead after the call.

                    assert(callInstrSize != 0);

                    if (args >= 0)
                    {
                        emitStackPop(dst, /*isCall*/ true, callInstrSize, args, /* isTailCall*/ ins != INS_call);
                    }
                    else
                    {
                ...
                if (!emitFullGCinfo && recCall && ins == INS_call)
                {
                    assert(callInstrSize != 0);
