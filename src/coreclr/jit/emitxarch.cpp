...
// clang-format on
{
    ...
    // TODO: VS should actually pass in when this is not a safe point,
    //       see: Compiler::IsGcSafePoint

    id->idSetIsNoGC(!isSafePoint || isJump || emitNoGChelper(methHnd));
    ...
    switch (insFmt)
    {
        ...
        DONE_CALL:
            ...
            // if (recCall || args)
            {
                ...
                if (args >= 0)
                {
                    emitStackPop(dst, /*isCall*/ true, recCall, callInstrSize, args);
                }
                else
                {
...
// Make sure we set the instruction descriptor size correctly
    ...
    if (updateStackLevel)
    {
        switch (ins)
        {
            ...
            case INS_pop:
                emitStackPop(dst, false, /*isSafepoint*/ false, /*callInstrSize*/ 0, 1);
                break;
            ...
            case INS_add:
                ...
                {
                    assert((size_t)emitGetInsSC(id) < 0x00000000FFFFFFFFLL);
                    emitStackPop(dst, /*isCall*/ false, /*isSafepoint*/ false, /*callInstrSize*/ 0,
